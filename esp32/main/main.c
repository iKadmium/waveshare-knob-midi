...
static void example_lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    ...
    *to++ = temp;
    ...
    // In LVGL 9.x, we must inform LVGL that flushing is complete
    // However, since esp_lcd_panel_draw_bitmap is async and will call our callback,
    // we should NOT call lv_display_flush_ready here - it's called in the callback
}
...
static lv_display_t *g_disp = NULL; // LVGL display object
...
void app_main(void)
{
    ...
    g_disp = lv_display_create(EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES);
    lv_display_set_flush_cb(g_disp, example_lvgl_flush_cb);
    lv_display_set_buffers(g_disp, buf1, buf2, EXAMPLE_LCD_H_RES * EXAMPLE_LVGL_BUF_HEIGHT * sizeof(lv_color_t), LV_DISPLAY_RENDER_MODE_PARTIAL);
    lv_display_set_user_data(g_disp, panel_handle);
    lv_display_add_event_cb(g_disp, example_lvgl_rounder_cb, LV_EVENT_INVALIDATE_AREA, NULL);

    // Set color format with byte swapping for correct RGB565 display
